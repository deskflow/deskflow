...
DWORD 
CArchDaemonWindows::getSessionId()
{
	return WTSGetActiveConsoleSessionId();
}

BOOL
CArchDaemonWindows::winlogonInSession(DWORD sessionId, PHANDLE process)
{
...
HANDLE 
CArchDaemonWindows::getCurrentUserToken(DWORD sessionId, LPSECURITY_ATTRIBUTES security)
{
	HANDLE currentToken;
...
int
CArchDaemonWindows::relaunchLoop()
{
	// not sure i like this too much, seems somewhat hacky
	const char* launchName = CArch::getInstance()->m_pname;

	// for trapping service launch in debugger
	...
	while (loopRunning) {
		...
		// only enters here when id changes, and the session isn't -1, which
		...
		if ((newSessionId != sessionId) && (newSessionId != -1)) {
			...
			// one recieving input from mouse and keyboard.
