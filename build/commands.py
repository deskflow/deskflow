...
def configure(argv, generator = None):
...
def persist_cmake():
	if sys.platform == 'win32':
		...
		version = '2.8.0'
		found_cmd = ''
		for test_cmd in (cmake_cmd, r'tool\cmake\bin\%s' % cmake_cmd):
			...
			if p.returncode == 0 and stdout == 'cmake version %s\r\n' % version:
				...
				if found_cmd == r'tool\cmake\bin\%s' % cmake_cmd:
					found_cmd = r'..\tool\cmake\bin\%s' % cmake_cmd
				break
