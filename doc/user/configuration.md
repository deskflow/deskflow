...
* `lockInput([seconds])`
: Experimental, macOS server only. Suppresses input from the server's shared keyboard, mouse and trackpad, whether the active computer is the server or a client. Releases previously forwarded keys and mouse buttons before reporting that input is locked. Hold Esc for three seconds, then release all keys and mouse buttons to resume sharing. The optional integer deadline (1–300 seconds) requests automatic release; without it, the lock lasts until Esc is held. Input during the lock is discarded.
: Configure this action in the server's text configuration, for example `keystroke(Control+Alt+Super+l) = lockInput(30)` in `section: options`. The shortcut uses the existing native key mapping. This action does not change power settings or prevent automatic sleep. Independent input devices on a client are outside its scope. Unlocking waits for the observed held keys and mouse buttons to be released. Quartz's source-state tables do not reflect swallowed input, so they are not used to infer releases during capture. If capture is interrupted, the local lock ends and sharing remains paused; stop automation, release the inputs, and restart Deskflow as indicated. If remote release fails, sharing remains paused: check and recover the client's input before restarting, rather than assuming its keys were released.
: Requires the matching experimental Windows client (protocol 1.9). The client releases its synthetic keys and shared buttons in place and reports the result; heartbeat echoes do not confirm release. Windows injection/delivery errors keep the session unconfirmed. Legacy 1.8 clients still support normal sharing, but the lock shortcut is rejected before capture closes when a known participant lacks the extension. Platforms without a checked release implementation advertise 1.8. A client that received input earlier remains a participant until a successful reset or disconnection; mixed-version sessions can therefore reject the lock even after keys are up. If an unsupported participant appears between the shortcut preflight and capture closing, the second check keeps sharing paused rather than claiming release. The updated client can also connect to a legacy 1.8 server for normal sharing.
: Test only with disposable windows, a short deadline and an independent process watchdog. A release result confirms the checked injection path, not that every application consumed the events. macOS system gestures, device mappings and Windows input recovery require physical validation before daily use. Locking also suppresses connection-triggered input/switch actions and hotkey deactivation actions. A missing observed release can keep sharing paused even after all inputs have been physically released; stop automation, check the client's input recovery, and restart Deskflow if sharing remains paused. With a responsive event loop, an unresponsive client can cause about seven seconds of local suppression (two seconds preparing, then five seconds releasing); this timeout does not reopen sharing without a release confirmation. The input-lock timer runs only while locking or recovering.
