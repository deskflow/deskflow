...
- For a quick start on building a client, see the **[Protocol Tutorial - Custom Client](protocol_tutorial.md)**.

## Protocol Overview

The Deskflow protocol enables keyboard and mouse sharing between multiple computers over TCP connections. It follows a client-server architecture where:

- **Server (Primary)**: Shares its input devices and manages screen layout
- **Client (Secondary)**: Receives input commands and executes them locally
...
```
                   ...
                   TCP Success│     │TCP Failure              │
                        ...
                        └─────┬─────┬─────┘                   │
                              │     │                         │
             Hello/HelloBack  │     │Version Mismatch         │
                       OK     │     └─────────────────────────┤
                              v                               │
                        ┌─────────────────┐                   │
                   ┌────┤   CONNECTED     │                   │
...
This table lists all protocol messages in order of typical usage flow:

| Message | Constant | Type | Direction | Purpose | Constraints | Protocol Version |
|---------|----------|------|-----------|---------|-------------|------------------|
| [**Handshake**](@ref protocol_handshake) |  |  |  |  |  |  |
...
**Modifier Remapping:**
- The server can command clients to remap modifier keys via the @ref protocol_input_events message
- The @ref protocol_message_flow function iterates through the options received in a @ref protocol_input_events message and updates the modifier translation table
...
- If the client fails to respond in time, the @ref protocol_error_handling function is triggered, the connection is logged as unresponsive, and the socket is closed

## Version Compatibility

| Version | Features | Compatibility |
|---------|----------|---------------|
| **1.0** | Basic keyboard/mouse sharing (@ref kMsgDKeyDown, @ref kMsgDMouseMove) | All versions |
| **1.1** | Physical key codes (@ref KeyButton) | 1.1+ |
| **1.2** | Relative mouse movement | 1.2+ |
| **1.3** | Keep-alive (@ref kMsgCKeepAlive), horizontal scroll (@ref kMsgDMouseWheel) | 1.3+ |
| **1.4** | Encryption support (@ref SecureSocket) | 1.4+ |
...
- [Protocol Tutorial - Custom Client](protocol_tutorial.md) – Client implementation example with cross-platform code
- @ref ProtocolTypes.h – Complete protocol specification
- @ref ProtocolUtil.h – Message formatting utilities
- @ref protocol_implementation – Client-side protocol implementation
...
See also the @ref protocol_implementation section for details on client-side protocol handling.
...
- [Protocol Tutorial - Custom Client](protocol_tutorial.md) – Step-by-step guide to implementing a client
- @ref ProtocolTypes.h – Complete protocol specification
- @ref ProtocolUtil.h – Message formatting utilities
- @ref protocol_implementation – Client-side protocol implementation
