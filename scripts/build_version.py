...
class Version:
    ...
    def isSamePatch(self, version):
        return (
            self.major.number == version.major.number
            and self.minor.number == version.minor.number
            and self.patch.number == version.patch.number
        )
...
def getVesionFromGit(cmakeVersion):
    try:
        taggedRevision = subprocess.check_output(
            ("git rev-list --tags --max-count=100").split(), universal_newlines=True
        )
...
def updateVersionFile(number):
    ...
    fp.write(
        content.replace(
            ...
            "set (SYNERGY_VERSION_BUILD " + str(number) + ")",
        )
    )
    fp.close()
