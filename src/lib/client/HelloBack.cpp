...
void HelloBack::Deps::invalidHello() { m_invalidHello(); }
...
void HelloBack::handleHello(
    deskflow::IStream *stream, const std::string &clientName) const {
  SInt16 serverMajor;
  SInt16 serverMinor;
  ...
  ProtocolUtil::readf(
      stream, kMsgHello, &protocolName, &serverMajor, &serverMinor);

  if (protocolName != kSynergyProtocolName &&
      protocolName != kBarrierProtocolName) {
    m_deps->invalidHello();
    return;
  }
  ...
  // check versions
  LOG_DEBUG(
      "got hello version %s, %d.%d", protocolName.c_str(), serverMajor,
      serverMinor);
  ...
  if (shouldDowngrade(serverMajor, serverMinor)) {
    LOG_NOTE(
        "downgrading to %d.%d protocol for server", serverMajor, serverMinor);
    helloBackMinor = serverMinor;
  } else if (
      serverMajor < m_majorVersion ||
      (serverMajor == m_majorVersion && serverMinor < m_minorVersion)) {
  ...
  ProtocolUtil::writef(
      stream, helloBackMessage.c_str(), helloBackMajor, helloBackMinor,
      &clientName);
}
