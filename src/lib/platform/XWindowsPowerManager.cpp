...
bool sleepInhibitCall(bool state,
                      ArchSystemUnix::InhibitScreenServices serviceID) {
  std::string error;
...
void XWindowsPowerManager::disableSleep() const {
  if (!sleepInhibitCall(true,
                        ArchSystemUnix::InhibitScreenServices::kScreenSaver) &&
      !sleepInhibitCall(
