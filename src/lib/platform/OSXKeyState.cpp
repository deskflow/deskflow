...
bool inputSourceLacksUnicodeKeyLayout(TISInputSourceRef source)
{
...
KeyButton OSXKeyState::mapKeyFromEvent(KeyIDs &ids, KeyModifierMask *maskOut, CGEventRef event) const
{
  ...
  AutoTISInputSourceRef inputSource(TISCopyCurrentKeyboardInputSource(), CFRelease);
  const bool imeWithoutLayout = inputSourceLacksUnicodeKeyLayout(inputSource.get());
  if (imeWithoutLayout) {
    m_deadKeyState = 0;
  }
  ...
  if (layoutValid) {
    ...
    // get the characters
    if (status == 0) {
      if (count != 0 || m_deadKeyState == 0 || imeWithoutLayout) {
        m_deadKeyState = 0;
