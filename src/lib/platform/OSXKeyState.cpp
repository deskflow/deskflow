...
bool OSXKeyState::getKeyMap(deskflow::KeyMap &keyMap, int32_t group, const IOSXKeyResource &r) const
{
  ...
  std::vector<ButtonKey> buttonKeys(r.getNumTables());

  // spacing characters generated by a dead key followed by space.  These
  // are layout-specific: for example, the Brazilian - Pro acute dead key
  // produces an ASCII apostrophe rather than a spacing acute accent.
  std::map<KeyID, deskflow::KeyMap::KeyItem> deadKeyOutputs;
  deskflow::KeyMap::KeyItem spaceItem;
  ...
  for (uint32_t i = 0; i < r.getNumButtons(); ++i) {
    ...
    std::set<std::pair<KeyID, KeyID>> keys;

    // convert the entry in each table for this button to a KeyID
    for (uint32_t j = 0; j < r.getNumTables(); ++j) {
      buttonKeys[j].m_id = r.getKey(j, i);
      buttonKeys[j].m_dead = deskflow::KeyMap::isDeadKey(buttonKeys[j].m_id);
      buttonKeys[j].m_deadKeyOutput = buttonKeys[j].m_dead ? r.getDeadKeyOutput(j, i) : kKeyNone;
    }

    // iterate over each character table
    for (uint32_t j = 0; j < r.getNumTables(); ++j) {
      // get the KeyID for the button/table
      KeyID id = buttonKeys[j].m_id;
      if (id == kKeyNone) {
        continue;
      }

      // A dead KeyID can have different spacing outputs under different
      // modifiers, so those are separate logical mappings.
      const auto key = std::make_pair(id, buttonKeys[j].m_deadKeyOutput);
      if (keys.count(key) > 0) {
        continue;
      }
      keys.insert(key);
      ...
      item.m_id = id;
      item.m_group = group;
      item.m_dead = buttonKeys[j].m_dead;
      item.m_client = buttonKeys[j].m_dead ? 1 : 0;
      deskflow::KeyMap::initModifierKey(item);
      if (item.m_lock) {
        // all locking keys are half duplex on OS X
      ...
      // Collect tables that map to the same KeyID and dead-key spacing
      // output.  We know there can't be earlier tables because of the check
      // above.
      std::set<uint8_t> tables;
      tables.insert(static_cast<uint8_t>(j));
      for (uint32_t k = j + 1; k < r.getNumTables(); ++k) {
        if (buttonKeys[k].m_id == id && buttonKeys[k].m_deadKeyOutput == buttonKeys[j].m_deadKeyOutput) {
      ...
      for (std::set<uint32_t>::iterator k = required.begin(); k != required.end(); ++k) {
        ...
        if (item.m_id == ' ' && !hasSpaceItem) {
          spaceItem = item;
          hasSpaceItem = true;
        } else if (item.m_dead && buttonKeys[j].m_deadKeyOutput != kKeyNone) {
          deadKeyOutputs.try_emplace(buttonKeys[j].m_deadKeyOutput, item);
        }
      }
    }
  }
  ...
  if (hasSpaceItem) {
    for (const auto &[output, deadKeyItem] : deadKeyOutputs) {
      const deskflow::KeyMap::KeyItemList sequence = {deadKeyItem, spaceItem};
      keyMap.addKeyCombinationEntry(output, group, sequence);
    }
  }
