...
protected:
  ...
  */
  virtual String doBGRFromIClipboard(
      const UInt8 *bgrData, UInt32 w, UInt32 h) const = 0;

  //! Convert from IClipboard format
  /*!
  Convert raw BGRA pixel data to another image format.
  */
  virtual String doBGRAFromIClipboard(
      const UInt8 *bgrData, UInt32 w, UInt32 h) const = 0;
  ...
  */
  virtual String doToIClipboard(
      const String &, UInt32 &w, UInt32 &h, UInt32 &depth) const = 0;
};
