...
// EVKey/IME corrupts when a kCGHIDEventTap listens for keyboard events, even if the
// callback returns immediately. Drop keyboard from the tap mask while on local screen.
CGEventMask keyboardEventMask()
{
  return CGEventMaskBit(kCGEventKeyDown) | CGEventMaskBit(kCGEventKeyUp) |
         CGEventMaskBit(kCGEventFlagsChanged);
}

CGEventMask mouseAndSystemEventMask()
{
  return kCGEventMaskForAllEvents & ~keyboardEventMask();
}
...
void OSXScreen::recreateEventTap(CGEventTapOptions option, CGEventMask mask)
{
  ...
  m_eventTapPort = CGEventTapCreate(kCGHIDEventTap, kCGTailAppendEventTap, option, mask, cb, this);
  if (!m_eventTapPort) {
    return;
  }
...
void OSXScreen::clearKeyState()
{
  if (m_keyState) {
    static_cast<OSXKeyState *>(m_keyState)->clearDeadKeyState();
  }
}
...
void OSXScreen::enable()
{
  ...
  // Primary starts on the local screen; secondary (client) starts off-screen.
  if (m_isPrimary) {
    recreateEventTap(kCGEventTapOptionDefault, mouseAndSystemEventMask());
  } else {
    hideCursor();
...
{
  OSXScreen *screen = (OSXScreen *)refcon;
  // Defense in depth while the tap mask is switched during enter()/leave().
