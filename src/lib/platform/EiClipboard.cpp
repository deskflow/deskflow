...
{
    auto timer = m_metrics ? m_metrics->startOperation(
        ClipboardOperationMetrics::Operation::Add, format) : nullptr;

    if (!m_open) {
        LOG_WARN("cannot add to clipboard, not open");
        if (timer) timer->setError("clipboard not open");
        return;
    }

    if (format < 0 || format >= kNumFormats) {
        LOG_WARN("invalid clipboard format: %d", format);
        if (timer) timer->setError("invalid format");
        return;
    }
    ...
    if (data.size() > m_maxDataSize) {
        ...
        if (timer) timer->setError("data too large");
        return;
    }

    if (timer) timer->setDataSize(data.size());

    // Validate and sanitize data
    if (!validateClipboardData(format, data)) {
        LOG_WARN("clipboard data validation failed for format %d", format);
        if (timer) timer->setError("validation failed");
        return;
    }
...
bool EiClipboard::validateClipboardData(EFormat format, const std::string& data) const
{
    ...
    switch (format) {
        ...
        case kBitmap: {
            ...
            if (data.substr(0, 2) == "\xFF\xD8") validHeader = true; // JPEG
            if (data.substr(0, 2) == "BM") validHeader = true; // BMP
            if (data.substr(0, 4) == "GIF8") validHeader = true; // GIF
...
{
    auto timer = m_metrics ? m_metrics->startOperation(
        ClipboardOperationMetrics::Operation::FormatNegotiation, format) : nullptr;
