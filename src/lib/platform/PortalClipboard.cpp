      ...
      m_type(type)
{
  ...
  connect(
      m_proxy, &PortalClipboardProxy::selectionTransferRequested, this, &PortalClipboard::onSelectionTransferRequested
  );
}
...
void PortalClipboard::onSelectionOwnerChanged(
    const QStringList &mimeTypes, bool sessionIsOwner, PortalClipboardProxy::SelectionType type
)
{
  if (type != m_type) {
    return;
  ...
  LOG_DEBUG(
      "Portal clipboard (%d) ownership changed: owner=%d, types=%s", (int)m_type, sessionIsOwner,
      mimeTypes.join(", ").toStdString().c_str()
  );
...
void PortalClipboard::onSelectionTransferRequested(
    const QString &mimeType, quint32 serial, PortalClipboardProxy::SelectionType type
)
{
  if (type != m_type) {
    return;
  ...
  LOG_DEBUG(
      "Portal requested selection transfer (%d) for %s (serial %u)", (int)m_type, mimeType.toStdString().c_str(), serial
  );
...
QString PortalClipboard::formatToMimeType(Format format) const
{
  ...
  case Format::HTML:
    return "text/html";
  case Format::Bitmap:
    return "image/png"; // Standard for Wayland
  default:
    return "";
  }
}
...
IClipboard::Format PortalClipboard::mimeTypeToFormat(const QString &mimeType) const
{
  ...
  if (mimeType.contains("image/png", Qt::CaseInsensitive) || mimeType.contains("image/jpeg", Qt::CaseInsensitive) ||
