...
EiClipboardNegotiator::EiClipboardNegotiator()
...
void EiClipboardNegotiator::addFormatPreference(const FormatPreference& preference)
{
    ...
    LOG_DEBUG("added format preference: format=%d, mime=%s, quality=%.2f",
              preference.format, preference.mimeType.c_str(), preference.quality);
}
...
void EiClipboardNegotiator::removeFormatPreference(IClipboard::EFormat format, const std::string& mimeType)
{
    ...
    if (it != m_preferences.end()) {
        ...
        prefs.erase(std::remove_if(prefs.begin(), prefs.end(),
            [&mimeType](const FormatPreference& pref) {
                return pref.mimeType == mimeType;
            }), prefs.end());
...
FormatPreference EiClipboardNegotiator::negotiateBestFormat(
    IClipboard::EFormat desiredFormat,
    const std::vector<std::string>& availableMimeTypes) const
{
    ...
    for (const auto& preference : preferences) {
        ...
        if (std::find(availableMimeTypes.begin(), availableMimeTypes.end(), 
                     preference.mimeType) == availableMimeTypes.end()) {
            continue;
        }
    ...
    LOG_DEBUG("negotiated format: %s (score: %.2f)", 
              bestPreference.mimeType.c_str(), bestScore);
    return bestPreference;
}

FormatPreference EiClipboardNegotiator::negotiateBestFormat(
    const std::vector<IClipboard::EFormat>& availableFormats) const
{
...
bool EiClipboardNegotiator::needsConversion(
    IClipboard::EFormat sourceFormat,
    const std::string& sourceMimeType,
    IClipboard::EFormat targetFormat,
    const std::string& targetMimeType) const
{
...
int EiClipboardNegotiator::getConversionPriority(
    IClipboard::EFormat sourceFormat,
    IClipboard::EFormat targetFormat) const
{
...
{
    m_weights = weights;
    LOG_DEBUG("format weights updated: quality=%.2f, efficiency=%.2f, compatibility=%.2f, lossless=%.2f",
              weights.quality, weights.efficiency, weights.compatibility, weights.lossless);
}
