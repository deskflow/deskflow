...
bool WaylandClipboard::checkCommandExists(const char *command)
{
  ...
  pid_t pid;
  int spawn_result =
      posix_spawnp(&pid, command, &file_actions, nullptr, const_cast<char *const *>(args.data()), environ);
...
std::string WaylandClipboard::executeCommand(const std::vector<const char *> &args) const
{
  ...
  pid_t pid;
  int spawn_result =
      posix_spawnp(&pid, args[0], &file_actions, nullptr, const_cast<char *const *>(args.data()), environ);
