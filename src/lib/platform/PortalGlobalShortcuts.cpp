...
void PortalGlobalShortcuts::bindShortcuts()
{
  ...
  for (const auto &binding : bindings) {
    g_ptr_array_add (shortcuts,
                     xdp_global_shortcut_new (binding.shortcutId.c_str(),
                                              binding.description.c_str(),
...
void PortalGlobalShortcuts::handleBindShortcuts(GObject *object, GAsyncResult *res)
{
  ...
  for (guint i = 0; i < shortcuts->len; i++) {
    ...
    LOG_DEBUG("global shortcut %s assigned to %s", xdp_global_shortcut_assigned_get_shortcut_id(shortcut), xdp_global_shortcut_assigned_get_trigger_description(shortcut));
  }
...
void PortalGlobalShortcuts::handleShortcutsChanged(XdpGlobalShortcutsSession *session, GPtrArray *shortcuts)
{
  ...
  for (guint i = 0; i < shortcuts->len; i++) {
    ...
    shortcut = static_cast<XdpGlobalShortcutAssigned *>(g_ptr_array_index(shortcuts, i));
    const char *shortcutId = xdp_global_shortcut_assigned_get_shortcut_id(shortcut);
    const char *triggerDescription = xdp_global_shortcut_assigned_get_trigger_description(shortcut);
