...
{
    LOG_DEBUG("clipboard sync initialized with compression %d, delta mode %d",
              static_cast<int>(m_config.compression), static_cast<int>(m_config.deltaMode));
}
    ...
    const std::string& baseHash)
{
            ...
            if (similarity >= m_config.deltaSimilarityThreshold) {
                ...
                if (deltaMode != DeltaMode::None) {
                    ...
                    if (deltaData.size() < processedData.size() * 0.8) { // Only use if significant savings
                        ...
                        usedDelta = true;
                        LOG_DEBUG("using delta compression: %zu -> %zu bytes (%.1f%% savings)",
                                  data.size(), deltaData.size(), 
    ...
    if (processedData.size() >= m_config.compressionThreshold) {
        ...
        if (compression != CompressionAlgorithm::None) {
            ...
            if (compressedData.size() < processedData.size() * 0.9) { // Only use if beneficial
                ...
                LOG_DEBUG("compressed data: %zu -> %zu bytes (%.1f%% savings)",
                          processedData.size(), compressedData.size(),
    ...
    LOG_DEBUG("prepared sync packet: %s, %zu -> %zu bytes, delta: %s, compression: %d",
              packet.id.c_str(), packet.originalSize, packet.data.size(),
              usedDelta ? "yes" : "no", static_cast<int>(packet.compression));
...
std::string EiClipboardSync::applySync(const SyncPacket& packet, const std::string& baseData)
{
    ...
    if (packet.deltaMode != DeltaMode::None && !packet.baseHash.empty()) {
        ...
        if (!base.empty()) {
            ...
            LOG_DEBUG("applied delta: %zu + %zu -> %zu bytes", 
                      base.size(), packet.data.size(), processedData.size());
        } else {
    ...
    LOG_DEBUG("applied sync packet: %s, %zu -> %zu bytes", 
              packet.id.c_str(), packet.data.size(), result.size());
    
    return result;
}

std::vector<uint8_t> EiClipboardSync::compress(
...
std::vector<uint8_t> EiClipboardSync::decompress(
...
double EiClipboardSync::calculateSimilarity(
    const std::vector<uint8_t>& data1,
    const std::vector<uint8_t>& data2) const
{
    ...
    for (size_t i = 1; i <= minLen - commonPrefix && 
         data1[data1.size() - i] == data2[data2.size() - i]; ++i) {
        commonSuffix++;
    }
...
DeltaMode EiClipboardSync::chooseDeltaMode(
    const std::vector<uint8_t>& oldData,
...
std::vector<uint8_t> EiClipboardSync::createBinaryDelta(
    const std::vector<uint8_t>& oldData,
    const std::vector<uint8_t>& newData) const
{
    ...
    while (newPos < newData.size()) {
        ...
        for (size_t searchPos = 0; searchPos <= oldData.size(); ++searchPos) {
            size_t length = 0;
            while (searchPos + length < oldData.size() && 
                   newPos + length < newData.size() &&
...
std::vector<uint8_t> EiClipboardSync::applyBinaryDelta(
    const std::vector<uint8_t>& baseData,
...
std::vector<uint8_t> EiClipboardSync::createTextDelta(
    const std::vector<uint8_t>& oldData,
    const std::vector<uint8_t>& newData) const
{
...
std::vector<uint8_t> EiClipboardSync::applyTextDelta(
    const std::vector<uint8_t>& baseData,
    const std::vector<uint8_t>& delta) const
{
...
std::vector<SyncPacket> EiClipboardSync::createProgressivePackets(const SyncPacket& largePacket) const
{
    ...
    for (size_t i = 0; i < totalChunks; ++i) {
        ...
        chunk.data = std::vector<uint8_t>(
            largePacket.data.begin() + start,
            largePacket.data.begin() + end);
