...
MSWindowsScreen::MSWindowsScreen(
  ...
  try {
    ...
    forceShowCursor();
    LOG(
        (CLOG_DEBUG "screen shape: %d,%d %dx%d %s",
         m_x,
         m_y,
         m_w,
         m_h,
...
bool MSWindowsScreen::leave() {
  ...
  if (m_isPrimary) {
    ...
    // warp to center
    LOG(
        (CLOG_DEBUG1 "warping cursor to center: %+d, %+d",
         m_xCenter,
         m_yCenter));
...
bool MSWindowsScreen::getThisCursorPos(LPPOINT pos) {
  ...
  LOG(
      (CLOG_DEBUG3 "%s Attempt: 1 , status %d, code: %d Pos {%d, %d}",
       __func__,
       result,
       error,
       pos->x,
...
bool MSWindowsScreen::setThisCursorPos(int x, int y) {
  ...
  auto error = GetLastError();
  LOG(
      (CLOG_DEBUG3 "%s Attempt: 1, status %d, code: %d",
       __func__,
       result,
       error));
  if (!result) {
    ...
    error = GetLastError();
    LOG(
        (CLOG_DEBUG3 "%s Attempt: 2, status %d, code: %d",
         __func__,
         result,
         error));
    updateDesktopThread();
...
bool MSWindowsScreen::onMouseMove(SInt32 mx, SInt32 my) {
  ...
  if (m_isOnScreen) {
    ...
    // secondary screen.
    LOG(
        (CLOG_DEBUG5 "warping server cursor to center: %+d,%+d",
         m_xCenter,
         m_yCenter));
...
void MSWindowsScreen::warpCursorNoFlush(SInt32 x, SInt32 y) {
  ...
  if (!isPrimary()) {
    if ((cursorPos.x != x) && (cursorPos.y != y)) {
      ...
      LOG(
          (CLOG_DEBUG "cursor pos %d, %d expected pos %d, %d",
           cursorPos.x,
           cursorPos.y,
           x,
           y));
