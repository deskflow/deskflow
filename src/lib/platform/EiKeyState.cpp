...
std::uint32_t EiKeyState::convertModMask(xkb_mod_mask_t xkbModMaskIn) const
{
  ...
  for (xkb_mod_index_t xkbModIdx = 0; xkbModIdx < xkb_keymap_num_mods(m_xkbKeymap); xkbModIdx++) {
    ...
    const auto levelThreeMapping = xkb_keymap_mod_get_mask(m_xkbKeymap, name);
#else
    ...
    const xkb_mod_index_t levelThreeIdx = xkb_keymap_mod_get_index(m_xkbKeymap, XKB_VMOD_NAME_LEVEL3);
    const xkb_mod_mask_t levelThree = UINT32_C(1) << levelThreeIdx;
    ...
    const xkb_mod_mask_t levelThreeMapping = xkb_state_serialize_mods(state, XKB_STATE_MODS_EFFECTIVE);
    xkb_state_unref(state);
#endif

    // Skip inactive modifiers.
    if ((xkbModMaskIn & levelThreeMapping) != levelThreeMapping)
      continue;
