...
void PortalRemoteDesktop::createSession(uint code, const QVariantMap &result)
{
  ...
  auto options = QVariantMap{
      ...
      {QStringLiteral("handle_token"), createToken()},
      {QStringLiteral("persist_mode"), s_portalPersistanceType},
  };

  auto restoreToken = Settings::value(Settings::Client::XdpRestoreToken).toString();
...
void PortalRemoteDesktop::sessionStarted(uint code, const QVariantMap &result)
{
  ...
  Settings::setValue(Settings::Client::XdpRestoreToken, result.value(QStringLiteral("restore_token")));
  Q_EMIT started();
}
