...
public:
    ...
    DeltaMode chooseDeltaMode(
        const std::vector<uint8_t>& oldData,
        const std::vector<uint8_t>& newData) const;

    //! Split large data into progressive transfer chunks
    std::vector<SyncPacket> createProgressivePackets(
        const SyncPacket& largePacket) const;

    //! Reassemble progressive transfer chunks
    SyncPacket reassembleProgressivePackets(
        const std::vector<SyncPacket>& packets) const;
...
private:
    ...
    std::vector<uint8_t> createBinaryDelta(
        const std::vector<uint8_t>& oldData,
        const std::vector<uint8_t>& newData) const;
    std::vector<uint8_t> applyBinaryDelta(
        const std::vector<uint8_t>& baseData,
        const std::vector<uint8_t>& delta) const;

    //! Text delta implementation
    std::vector<uint8_t> createTextDelta(
        const std::vector<uint8_t>& oldData,
        const std::vector<uint8_t>& newData) const;
    std::vector<uint8_t> applyTextDelta(
        const std::vector<uint8_t>& baseData,
        const std::vector<uint8_t>& delta) const;
