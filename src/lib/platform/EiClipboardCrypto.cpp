      ...
      m_lastKeyRotation(std::chrono::system_clock::now())
{
    ...
    LOG_DEBUG("clipboard crypto initialized with algorithm %d", 
              static_cast<int>(m_config.algorithm));
}
...
bool EiClipboardCrypto::validateIntegrity(const EncryptedData& data) const
{
    ...
    return !data.ciphertext.empty() && 
           (data.algorithm == EncryptionAlgorithm::None || !data.keyId.empty());
}
...
ClipboardCryptoScope::ClipboardCryptoScope(std::shared_ptr<EiClipboardCrypto> crypto, const std::string& data)
    : m_crypto(crypto), m_originalData(data), m_encrypted(false)
{
