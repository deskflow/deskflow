...
EiClipboardHistory::EiClipboardHistory(const Config& config)
    : m_config(config), m_totalSize(0)
{
    LOG_DEBUG("clipboard history initialized with max %zu entries, %zu bytes",
              m_config.maxEntries, m_config.maxTotalSize);
}
    ...
    const std::string& source)
{
    ...
    // Create new entry
    auto entry = std::make_shared<ClipboardHistoryEntry>(
        format, data, hash, containsSensitiveData, source);
    ...
    LOG_DEBUG("added clipboard history entry: format=%d, size=%zu, hash=%s",
              format, entry->size, hash.c_str());
