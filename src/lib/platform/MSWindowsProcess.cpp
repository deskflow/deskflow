...
void MSWindowsProcess::closeHandles(PROCESS_INFORMATION &info)
{
...
BOOL MSWindowsProcess::startInForeground()
{
  ...
  return CreateProcess(nullptr, LPSTR(m_command.c_str()), nullptr, nullptr, TRUE, 0, nullptr, nullptr, &si, &m_info);
}
...
std::string MSWindowsProcess::readOutput(HANDLE handle)
{
  ...
  if (!ReadFile(handle, buffer, kOutputBufferSize, &bytesRead, nullptr)) {
    LOG_ERR("could not read from error pipe");
    throw XArch(new XArchEvalWindows());
  }
