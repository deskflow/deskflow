...
{
    LOG_DEBUG("clipboard metrics initialized (enabled: %s)", 
...
void EiClipboardMetrics::recordOperation(const ClipboardOperationMetrics& metrics)
{
    ...
    LOG_DEBUG("recorded clipboard operation: %d, format: %d, duration: %lld μs, size: %zu",
              static_cast<int>(metrics.operation), metrics.format, 
              metrics.duration.count(), metrics.dataSize);
}

EiClipboardMetrics::OperationTimer::OperationTimer(
    EiClipboardMetrics* metrics,
...
EiClipboardMetrics::OperationTimer::~OperationTimer()
{
    if (m_metrics) {
        ...
        m_operation.duration = std::chrono::duration_cast<std::chrono::microseconds>(
            endTime - m_startTime);
...
std::unique_ptr<EiClipboardMetrics::OperationTimer> EiClipboardMetrics::startOperation(
    ClipboardOperationMetrics::Operation operation,
    IClipboard::EFormat format)
{
    if (!m_config.enabled) {
...
PerformanceStats EiClipboardMetrics::getPerformanceStats(ClipboardOperationMetrics::Operation operation) const
{
    ...
    std::copy_if(m_metrics.begin(), m_metrics.end(), std::back_inserter(filtered),
        [operation](const ClipboardOperationMetrics& m) {
...
PerformanceStats EiClipboardMetrics::getPerformanceStats(IClipboard::EFormat format) const
{
    ...
    std::vector<ClipboardOperationMetrics> filtered;
    std::copy_if(m_metrics.begin(), m_metrics.end(), std::back_inserter(filtered),
        [format](const ClipboardOperationMetrics& m) {
...
std::vector<ClipboardOperationMetrics> EiClipboardMetrics::getRecentMetrics(size_t count) const
{
    ...
    return std::vector<ClipboardOperationMetrics>(
        m_metrics.end() - count, m_metrics.end());
}
    ...
    std::chrono::steady_clock::time_point end) const
{
    ...
    std::vector<ClipboardOperationMetrics> filtered;
    std::copy_if(m_metrics.begin(), m_metrics.end(), std::back_inserter(filtered),
        [start, end](const ClipboardOperationMetrics& m) {
...
void EiClipboardMetrics::cleanupOldMetrics()
{
    ...
    auto it = std::remove_if(m_metrics.begin(), m_metrics.end(),
        [cutoff](const ClipboardOperationMetrics& m) {
