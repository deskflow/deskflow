...
void Server::onMouseMoveSecondary(int32_t dx, int32_t dy)
{
  ...
  // adjust mouse speed
  dx = static_cast<int32_t>(std::round(dx * m_args.m_mouseSpeed));
  dy = static_cast<int32_t>(std::round(dy * m_args.m_mouseSpeed));
  LOG((CLOG_DEBUG2 "set mouse speed to %f", m_args.m_mouseSpeed));
