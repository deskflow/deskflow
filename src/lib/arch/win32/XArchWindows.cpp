...
{
  // Use FormatMessage via windowsErrorToQString for accurate, localized error messages.
  // The previous hardcoded error list was often inaccurate (e.g., WSAEACCES always showed
  // "broadcast address" message even when caused by firewall blocking). Fixes #9067.
  return windowsErrorToQString(static_cast<DWORD>(error));
}
