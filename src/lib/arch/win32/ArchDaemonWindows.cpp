...
{
  assert(s_daemon != nullptr);
  throw ArchDaemonExceptionRunFailed(result);
}
...
)
{
  ...
  if (mgr == nullptr) {
    ...
    throw ArchDaemonInstallFailedException(windowsErrorToString(GetLastError()));
  }
  ...
  if (service == nullptr) {
    ...
    if (err != ERROR_SERVICE_EXISTS) {
      ...
      throw ArchDaemonInstallFailedException(windowsErrorToString(err));
    }
  } else {
...
void ArchDaemonWindows::serviceMain(DWORD argc, LPTSTR *argvIn)
{
  ...
  } catch (ArchDaemonExceptionRunFailed &e) {
