...
)
{
  ...
  // set the description
  ArchMiscWindows::setValue(key, _T("Description"), qPrintable(description));
  ...
  ArchMiscWindows::setValue(key, _T("CommandLine"), qPrintable(commandLine));
...
{
  LOG_DEBUG("uninstalling windows service: %s", qPrintable(name));
