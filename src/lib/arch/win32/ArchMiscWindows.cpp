...
HMODULE ArchMiscWindows::findLoadedModule(std::array<const wchar_t *, 2> moduleNames)
{
  ...
  for (size_t i = 0; i < (cbNeeded / sizeof(HMODULE)); ++i) {
    ...
    for (const auto &moduleName : moduleNames) {
      if (_wcsicmp(reinterpret_cast<wchar_t *>(loadedModuleName.data()), moduleName) == 0) {
        return hModules[i];
      }
    }
  }
...
{
  auto hModule = findLoadedModule({L"vcruntime140.dll", L"vcruntime140d.dll"});
  if (hModule == nullptr) {
