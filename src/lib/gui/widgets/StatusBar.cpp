...
void StatusBar::setServerClients(const QStringList &clients)
{
  ...
  m_lblStatus->setText(
      tr("%1 is connected, with %n client(s): %2", "", clients.size()).arg(kAppName, clients.join(", "))
  );
...
bool StatusBar::securityIconVisible()
{
...
void StatusBar::updateSecurityInfo(bool encrypted, const QString &secureSocketVersion)
{
  const auto txt = encrypted ? tr("%1 Encryption Enabled").arg(secureSocketVersion) : tr("Encryption Disabled");
