...
#else
  QSKIP("Wayland clipboard not available on this platform");
#endif
}
...
void WaylandClipboardTests::singleFormat()
{
  ...
  if (!waitForClipboardContent(clipboard, IClipboard::kText, m_testString)) {
    clipboard.close();
...
void WaylandClipboardTests::multipleFormats()
{
  ...
  QVERIFY(waitForClipboardContent(clipboard, IClipboard::kText, m_testString));
  ...
  clipboard.add(IClipboard::kText, m_testString2);
  QVERIFY(waitForClipboardContent(clipboard, IClipboard::kText, m_testString2));
...
void WaylandClipboardTests::hasFormat()
{
  ...
  QVERIFY(!clipboard.has(IClipboard::kText));
  QVERIFY(!clipboard.has(IClipboard::kHTML));
  QVERIFY(!clipboard.has(IClipboard::kBitmap));

  // Add text and verify
  clipboard.add(IClipboard::kText, m_testString);
  QVERIFY(waitForClipboardContent(clipboard, IClipboard::kText, m_testString));
  QVERIFY(clipboard.has(IClipboard::kText));
  QVERIFY(!clipboard.has(IClipboard::kHTML)); // HTML not supported
  QVERIFY(!clipboard.has(IClipboard::kBitmap));
...
void WaylandClipboardTests::monitoring()
{
  ...
  if (externalClipboard.open(1)) {
    ...
    externalClipboard.add(IClipboard::kText, m_testString);
...
void WaylandClipboardTests::primaryClipboard()
{
  ...
  clipboard.add(IClipboard::kText, m_testString);
  primaryClipboard.add(IClipboard::kText, m_testString2);

  // Wait for and verify they contain different data
  QVERIFY(waitForClipboardContent(clipboard, IClipboard::kText, m_testString));
  QVERIFY(waitForClipboardContent(primaryClipboard, IClipboard::kText, m_testString2));
...
void WaylandClipboardTests::getWithoutOpen()
{
  ...
  std::string result = clipboard.get(IClipboard::kText);
...
)
{
  auto condition = [&clipboard, &previousContent]() {
    ...
    std::string currentContent = clipboard.get(IClipboard::kText);
...
bool WaylandClipboardTests::waitForClipboardContent(
    WaylandClipboard &clipboard, IClipboard::EFormat format, const std::string &expectedContent, int timeoutMs
)
{
