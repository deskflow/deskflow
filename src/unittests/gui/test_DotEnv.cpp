...
#include "test_DotEnv.h"

void DotEnv_Test::initTestCase()
{
  QDir dir;
  QVERIFY(dir.mkpath("tmp/test"));
}

void DotEnv_Test::invalidFile()
{
...
void DotEnv_Test::validFile()
{
  ...
  QCOMPARE("test value", actualValue.toStdString());

  QVERIFY(QFile::remove(m_envFile));
}

QTEST_MAIN(DotEnv_Test)
