...
BonjourWindows::BonjourWindows(SettingsDialog *settingsDialog,
                               MainWindow *mainWindow, AppConfig &appConfig)
    : m_pSettingsDialog(settingsDialog), m_pMainWindow(mainWindow),
      m_pBonjourInstall(nullptr), m_pDownloadMessageBox(nullptr),
      m_pDataDownloader(nullptr), m_appConfig(appConfig) {}
...
void BonjourWindows::downloadAndInstall() {
  ...
  if (arch == kProcessorArchWin32) {
    ...
    QMessageBox::critical(m_pSettingsDialog, tr("Synergy Auto Config"),
  ...
  QAbstractButton *cancel =
      m_pDownloadMessageBox->addButton(tr("Cancel"), QMessageBox::RejectRole);
...
bool BonjourWindows::isRunning() const {
  ...
  if (hSCManager == nullptr) {
    m_pMainWindow->appendLogError(
        QString("failed to open a service controller manager, error: %1")
            .arg(GetLastError()));
    return false;
  }
