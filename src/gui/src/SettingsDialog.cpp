...
#if defined(Q_OS_WIN)
    m_pComboElevate->setCurrentIndex (
        static_cast<int> (appConfig ().elevateMode ()));
...
void
SettingsDialog::changeEvent (QEvent* event) {
    if (event != 0) {
        switch (event->type ()) {
...
void
SettingsDialog::on_m_pCheckBoxLogToFile_stateChanged (int i) {
    bool checked = i == 2;
...
void
SettingsDialog::on_m_pButtonBrowseLog_clicked () {
    QString fileName =
...
void
SettingsDialog::on_m_pComboLanguage_currentIndexChanged (int index) {
    QString ietfCode = m_pComboLanguage->itemData (index).toString ();
    QSynergyApplication::getInstance ()->switchTranslator (ietfCode);
}

void
SettingsDialog::on_m_pCheckBoxEnableCrypto_toggled (bool checked) {
