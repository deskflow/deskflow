...
AboutDialogEliteBackers::AboutDialogEliteBackers(MainWindow *parent,
                                                 const AppConfig &config)
    : AboutDialog(parent, config), credits(*parent, config) {
  setupCreditsLoader();
...
void AboutDialogEliteBackers::setupCreditsLoader() {
  ...
  connect(&credits, SIGNAL(loaded(const QString &)), this,
          SLOT(updateEliteBackers(const QString &)));
  credits.loadEliteBackers();
}

void AboutDialogEliteBackers::updateEliteBackers(
    const QString &eliteBackers) const {
...
QString AboutDialogEliteBackers::getCopyrights() const {
  ...
  QString aboutText(
      R"(<p>Keyboard and mouse sharing application. Cross platform and open source since 2001.<br /><br />
                        Copyright © %%YEAR%% Symless Ltd.<br /><br />
                        Synergy is released under the GNU General Public License (GPLv2).</p>)");

  return aboutText.replace(QString("%%YEAR%%"),
