...
public:
  ...
  bool isSuccess() const { return (m_status == "success"); }
...
CreditsLoader::CreditsLoader(MainWindow &mainWindow, const AppConfig &config)
    : m_mainWindow(mainWindow), m_config(config) {
...
void CreditsLoader::loadEliteBackers() {
  ...
  m_mainWindow.appendLogDebug(
      QString("Send request: %1").arg(ELITE_BACKERS_URL));
...
void CreditsLoader::replyFinished(QNetworkReply *reply) const {
  ...
  m_mainWindow.appendLogDebug(
      QString("Received response: %1").arg(response.toStdString().c_str()));
...
void CreditsLoader::error(const QString &error) const {
  ...
  emit loaded("Something went wrong when retrieving the list of Elite Backers. "
