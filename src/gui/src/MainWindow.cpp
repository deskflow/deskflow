...
#ifdef Q_OS_MAC
    // On mac os the titlebar is part of the height so we need to adjust our Y coord to not "jump" up
    setGeometry(x(), y() + 28, m_logRect.width(), m_logRect.height());
#else
    setGeometry(x(), y(), m_logRect.width(), m_logRect.height());
#endif
  } else {
    adjustSize();
