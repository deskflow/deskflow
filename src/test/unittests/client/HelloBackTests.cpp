...
void setupMockHelloRead(
    MockStream &stream, const std::string &protocolName,
    const SInt16 majorVersion, const SInt16 minorVersion) {
...
void setupMockHelloBackWrite(
  ...
  const UInt32 helloBackSize = static_cast<UInt32>(
      protocolName.size() + versionIntSize + clientNameIntSize + name.size());
  ...
  EXPECT_CALL(stream, write(_, helloBackSize))
      .WillOnce(WithArg<0>([expect, helloBackSize](const void *vbuffer) {
